...
## [2026-05-28] feature(branch) | Multi-round interviews prototype (Approach A) on feature/multi-round-interviews
attributed_to: [niko, claude]   belongs_to: [tecxwork, recruitment-workflows, bookings]
- Built on branch `feature/multi-round-interviews` — **main untouched** at niko's request. Motivation: a friend at Michael Page noted that good ATS/application-management systems let recruiters run a candidate through multiple interview rounds and control the timeline.
- Approach A (chained bookings), ad-hoc rounds — chosen over a full ATS pipeline (Approach B: separate `applications` + `interview_rounds` tables) to prototype fast by reusing the existing booking engine. A candidate's pipeline = bookings sharing applicant+recruiter(+position), linked by `parent_booking_id`, ordered by `round_number`.
- Schema (migration `drizzle/0006_multi_round_interviews.sql`, hand-written + journal entry since `drizzle-kit generate` needs a TTY): additive columns on `bookings` — `round_number` (default 1), `round_label`, `parent_booking_id` (self-FK ON DELETE SET NULL), `outcome` (advanced|hired|rejected|no_show). **Migration NOT yet applied to any database** — additive/backwards-compatible, so it's safe to share a DB with main, but apply deliberately.
- Endpoints: `POST /api/bookings/[id]/next-round` (recruiter advances an accepted booking → creates next-round booking in `reschedule_proposed` status with the proposed time, marks parent `outcome=advanced`; student accepts/declines via the EXISTING `/respond-proposal` slot-claim flow, zero changes there). `PATCH /api/bookings/[id]/outcome` records a round verdict.
- UI: recruiter dashboard shows a "Round N · label" badge + outcome badge on cards; accepted cards get "Next round" (opens label+time modal) and "Mark hired" actions.
- Known prototype gaps: no cross-company conflict guard on next-round time (the propose-time guard isn't reused here); "application/pipeline" is a derived grouping not a first-class entity; student-side UI shows the next round as a generic reschedule proposal (no round label yet); Mode-A bookings with null applicantId won't surface the next round to the student. Graduate to Approach B if the concept proves valuable.
