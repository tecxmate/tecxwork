...
## [2026-09-14] decision | marketecx: tecxwork contributes aggregates, and the design system's missing dark mode
attributed_to: [niko, claude-code]   belongs_to: [marketecx, tecxwork]
- [niko] is joining bizmap, alphatecx and tecxwork into **marketecx**, a market-intelligence product reached through a queryable Claude connector — "ask what restaurants are on 永康街, query both Google Maps and our data, compare". All three codebases read; the design is written up in `tecxmate/bizmap` → `docs/marketecx.md` ([PR #32](https://github.com/tecxmate/bizmap/pull/32)), not started.
- **tecxwork's boundary is set by `consent_purpose`, not by aggregation downstream.** `applicant_profiles` records consent for recruitment for a named event and employer; market intelligence is not that purpose, so only derived counts leave this repo — a `/api/market-pulse` on the `event-pulse` pattern (public, CORS, rate-limited, cached, minimum cell size).
- **`clients.unified_business_no` is the join key and is currently unverified free text.** Backfilling it against bizmap's registry also gives tecxwork verified client identity, which the ESA-licensee compliance story wants on its own terms. alphatecx, by contrast, holds **no 統編 at all** — one column on its `dim_ticker` is the cheapest change in the whole plan.
- **The justification for tecxwork's involvement is a hypothesis, and it is testable**: openings here lead `raw_monthly_revenue` there, using alphatecx's existing `q_lead_lag`. Recorded as something to test on history rather than assume — if it fails, tecxwork's role is much smaller.
- **Found while working in bizmap: the Tecxmate design system document defines no dark palette.** `src/app/globals.css`'s `.dark` block is the only place a Tecxmate dark theme exists anywhere, so bizmap's admin panel had to source its dark half from this repo. One published `tecxmate-tokens.css` carrying both modes would stop each new surface re-deriving a different dark theme.
- created [decisions/2026-09-14-marketecx-tecxwork-contribution.md](decisions/2026-09-14-marketecx-tecxwork-contribution.md), [topics/marketecx.md](topics/marketecx.md); index updated. No code changed in this repo.

## [2026-09-15] decision | bizmap/marketecx is the flagship; tecxwork feeds it and is not sold
attributed_to: [niko]   belongs_to: [tecxwork, marketecx, saas-strategy]
- [niko]: "monetizing tecxwork and alphatecx both touch legal gray zone, probably need to leverage their data to support bizmap only, bizmap, marketecx system is our flagship product."
- **Which gray zone, exactly, is recorded rather than assumed** — the two readings lead to different work. Taken: [taiwan-compliance](topics/taiwan-compliance.md) §1, an ESA licence is needed for matching employers and job seekers **for profit**, and §1's avoidance strategy is scoped explicitly to "the unlicensed Tecxmate independent product". Same shape as alphatecx's investment-advice exposure: Tecxmate selling a regulated activity without holding the licence.
- **What that leaves standing:** [yang-luck] *is* the ESA licensee, and [2026-08-12](decisions/2026-08-12-saas-tenancy-and-commercial-model.md) sells per-seat software to licensed agencies, chosen partly because it "fits ESA licensing". Selling software to a licensee is not unlicensed matching-for-profit. The wider reading — tecxwork stops being sold at all — would supersede 2026-08-12 and change what the Yang Luck build is for, so it is flagged for [niko] rather than inferred.
- **The principle across all three systems: sell where the licence is already held, or not needed.** bizmap is the only one whose basis is affirmative — 政府資料開放授權條款 不限目的 permits commercial use by name, rather than merely failing to forbid it. That is a better reason to make it the flagship than product-market fit.
- **Corollary, easy to get wrong: a licence travels with the data, not with the repo it sits in.** Not being sold here does not make an applicant-derived figure sellable by aggregating it into something that is — so the boundary stays at this repo's API, which is what 09-14 already decided.
- No build change. The aggregates-only rule was set by `consent_purpose` and no commercial decision moves it; it gains a second reason, and the minimum cell size becomes load-bearing because counts inside a *sold* product are a harder §20 question.
- created [decisions/2026-09-15-flagship-is-bizmap-tecxwork-feeds-it.md](decisions/2026-09-15-flagship-is-bizmap-tecxwork-feeds-it.md); updated [decisions/2026-09-14-marketecx-tecxwork-contribution.md](decisions/2026-09-14-marketecx-tecxwork-contribution.md), [topics/saas-strategy.md](topics/saas-strategy.md), [index.md](index.md). No code changed.

## 2026-09-21 — AI screening interviewer (tecxwork)

- [niko] asked for a text-only AI chatbot interviewer that judges an applicant against a job description, examines them strictly against their CV, and does not take their claims on trust. Demo scope; visual and TTS bots are named as later.
- **The part that makes it a real problem, in his words:** 「Many people have made interview bots before, but also the applicants can outsmart it by many ways such as opening a new website or a new AI to counter that in real time … they might have another TTS on their side to listen to everything and help them answer just reading the script.」 He is right, and right that voice does not fix it — a TTS relay is the same attack with more latency, which is why text-only costs the demo nothing.
- **Decision: treat coaching as an information problem, not a detection problem.** A detection-first design ranks candidates by how suspicious their typing looked, which punishes a phone keyboard and a Vietnamese IME far more reliably than it catches anyone. Instead every question is built to need something the helper does not have — the CV, the earlier transcript, a constraint invented thirty seconds ago, or the candidate's own judgement about their own mistake. Taken: [decisions/2026-09-21-ai-screening-interviewer.md](decisions/2026-09-21-ai-screening-interviewer.md).
- **The mechanism that carries it is the claim ledger**, re-probing an assertion 5+ turns later from a new angle: answering that needs the whole transcript re-pasted every turn, against a clock. Word caps are clamped in code rather than left to the model, so a persuasive candidate cannot talk the interviewer into a generous one.
- **False premises are bounded at two per interview and never before question four** — the technique works by being unexpected, and a candidate who has met three hedges everything, destroying the signal in every other question. Playing along is recorded as a flag, never a verdict: people are agreeable under pressure, especially across a language barrier and towards someone who sounds like the employer.
- **Telemetry never decides anything.** Counts and durations only, each signal stating the measurement rather than a conclusion, the caveat travelling with the score, and the verdict escalating on the *second* high-severity flag — one flag means "read that question", and a verdict that fires constantly is one nobody reads.
- **The integrity half is deterministic on purpose.** A flag a model produced could not be explained to a candidate who disputes it, and a ledger a model keeps is one that can quietly stop checking anything while the report still looks the same.
- First AI provider in this repo: `@anthropic-ai/sdk`, `claude-opus-5`, chosen by [niko] over mirroring tecxmate's OpenAI chatbot. `applications.ai_score` stops being a seeded placeholder on rows that have been interviewed.
- created [decisions/2026-09-21-ai-screening-interviewer.md](decisions/2026-09-21-ai-screening-interviewer.md), [topics/ai-interviewer.md](topics/ai-interviewer.md); updated [index.md](index.md). Code: `src/lib/interview/`, `src/app/interview/[token]/`, `src/app/api/ai-interviews/`, `src/components/ai-interview-panel.tsx`, two new tables.

## 2026-09-22 — half the dashboard's database round trips were the same three questions

- [niko]: 「Go do anything to optimize the tecxwork system.」 No target named, so the target was measured: local Postgres 16 with `log_statement=all`, the Yang Luck seed, a production build, and a script bracketing each request with marker queries.
- **Finding: 280 statements across thirteen dashboard pages, about half exact repeats — and every repeat was auth or tenancy context.** One render of `/dashboard/clients` read the session row six times and the recruiter row four. Taken: [decisions/2026-09-22-memoise-request-scoped-auth-lookups.md](decisions/2026-09-22-memoise-request-scoped-auth-lookups.md).
- **What was NOT wrong, which set the scope:** no N+1 in any request path (every awaited-query-in-a-loop site is a seed script), and the loaders already run concurrently across several connections. Duplication, not serialisation or fan-out.
- **The duplication is a consequence of a property worth keeping** — every loader resolves its own auth, which is what stops a page rendering rows the equivalent route would refuse. Memoising keeps the property and drops the cost; unpicking it would have traded a latency win for the class of bug `agency-auth.ts` exists to document.
- **The keying rule is the security story, not a style note.** Every memo takes the id it looks up as an argument; none read cookies or headers. A memo keyed on nothing would hand request one's session to request two if React's per-request scope ever failed, and every single-account test would still pass. `getSession()` is therefore not memoised — only its row check is.
- Verified rather than assumed: 400 requests / 4 users / 40-way concurrency with every response checked against its own cookie (zero leaks); 150 interleaved page loads hashed per identity (no body shared); revocation and expiry still bite on the next request; the RSC payload byte-identical before and after once reconstructed from the stream. 302 tests pass.
- **280 → 133 statements (−52%).** `/dashboard/clients` 28→12, `/dashboard/placements` 30→14.
- Environment gotcha, cost twenty minutes: a throwaway probe route at `src/app/api/__probe` silently 404s — the App Router treats a `_`-prefixed folder as private.
- created [decisions/2026-09-22-memoise-request-scoped-auth-lookups.md](decisions/2026-09-22-memoise-request-scoped-auth-lookups.md), [topics/request-memoisation.md](topics/request-memoisation.md); updated [index.md](index.md). Code: `src/lib/request-cache.ts`, `auth.ts`, `ats-auth.ts`, `tenant.ts`, `agency-auth.ts`, `agency-crm.ts`, `pipeline-report.ts`, `src/test/request-cache.test.ts`.
