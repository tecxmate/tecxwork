...
## [2026-08-16] decision | Manual screenshots rebranded by repainting, not re-capture
attributed_to: [niko]   belongs_to: [platform-manual]
- niko: *"Replace the Yang Luck logo to TECXMATE branding ... you don't have to take the screenshot all over again."* Answer to "did we update the documentation screenshots" — no, they are untouched since 08-11 and predate the brand fix, the rails, /how-it-works and the mobile nav.
- **49 header lockups and 62 footer lines repainted in place.** Replacement art is rendered by the same browser at capture-all.mjs's DSF 2 and downscaled by optimize.py's 2100/2880, from the real Instrument Serif and Geist in the Next font cache — so it resamples like its neighbours instead of looking pasted on.
- **Anchoring on the logo's red alone was wrong**: the job-moderation *Reject* buttons are red too and matched 210 times. Each anchor is now verified against a template cut from a known-good lockup — real ones score 0.4–3.1 mean grey levels, buttons nowhere near.
- Every fill was flatness-checked first (worst frame deviation 0.63 grey levels), so no border or gradient was flattened. Re-encoded at optimize.py's own q78/method 6; the set shrank 7.1 → 6.5 MB.
