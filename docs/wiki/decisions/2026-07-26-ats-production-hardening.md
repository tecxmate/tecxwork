...
- **Phase 5 (PII governance) — DONE + verified live (2026-07-27).** Commit `ac3f1a4`. Migration `db:update:ats-pii`: `applicant_profiles` + consent_at / consent_purpose / retention_until / anonymized_at; backfilled consent + 18-month retention for 37 candidates. Timeline API returns candidate governance (consent date, retention date + review-due flag, canErase by role). `POST /api/applications/:id/erase-candidate` = org-manager-only right-to-erasure (anonymizes PII in place, keeps row + non-PII aggregates, audited). CandidateTimeline "Data & consent" panel + Erase-PII action (managers only, confirm). Verified: consent/retention shown; unauth erase→401, non-manager→403, canErase true only for agency; 0 accidental erasures.
  - **Phase 5 remaining (TODO):** automated retention enforcement (auto-anonymize on retention_until), cross-border transfer register, client portal. Talent pools + signed-URL doc storage also still open (Phase 3 leftovers).
