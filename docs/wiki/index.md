...
## Decisions
- [2026-09-22 — memoise auth and tenancy per request, keyed on an identifier and never on the cookie](decisions/2026-09-22-memoise-request-scoped-auth-lookups.md) — measured 280 statements across 13 dashboard pages, ~half exact repeats of session/recruiter/org context; `cache()` per request cuts it to 133. Keyed on ids rather than the cookie, so a scope bug cannot cross users; a test fails the build on an unkeyed memo
- [2026-09-21 — the AI interviewer defeats coaching by asking what the coach cannot answer](decisions/2026-09-21-ai-screening-interviewer.md) — [niko]: applicants can put any question into a second AI in real time, and TTS does not help. So the defence is informational, not forensic: every question needs the CV, the earlier transcript, a constraint just invented, or the candidate's own judgement — none of which a helper has. Typing telemetry is the last line and never a verdict
...
- [2026-08-12 — The /documentation route is closed; the manual ships as a file](decisions/2026-08-12-documentation-route-closed.md) — removing the rewrite alone would not have closed it (static file in public/ was served directly); builds now emit outside public/ so a rebuild cannot silently re-expose. Git history and the public repo still hold the screenshots
- [2026-08-12 — /how-it-works: the employer- and agency-facing explainer](decisions/2026-08-12-how-it-works-page.md) — argues mechanism over features; records five tempting-but-false claims (AI screening, client portal, one-click RBA export, a single expiry window, 30-min slots) that a code survey ruled out. English only for now
- [2026-08-12 — Brand, notifications and account move into the workspace rail](decisions/2026-08-12-workspace-rail-brand-and-account.md) — standard SaaS corner: company name over role, notifications above the account, desktop top bar removed as empty. Menu-close-on-collapse derived rather than set in an effect
- [2026-08-12 — The mobile nav pill scrolls horizontally](decisions/2026-08-12-mobile-nav-scrollable.md) — grid-of-equal-columns gave an agency ~30px per tab; now a scrolling flex row. Surfaced a pre-existing bug where the active pill never rendered on a cold load (effect read refs before the mount gate flipped)
...
- [Per-request memoisation of auth and tenancy](topics/request-memoisation.md) — `src/lib/request-cache.ts`, the keying rule that makes it safe, the before/after statement counts, and how isolation was verified
...
- [AI screening interviewer](topics/ai-interviewer.md) — text-only Claude interviewer off a Pipeline card: CV-grounded drilling, a claim ledger re-probed across turns, bounded false premises, per-question clocks, and composition telemetry that flags rather than judges
