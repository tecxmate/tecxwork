...
## Event-Day Visualizer
- **2026-06-06**: Created `public/event-pulse.html`, a participant-facing standalone HTML screen that uses Tecxmate design-system tokens, animated platform-signal visuals, a Taipei live clock, live public recruiter/company fetch where available, and simulated private booking/notification pulses. True real-time booking metrics require a future aggregate endpoint that exposes counts only.
- **2026-06-06**: Expanded the visualizer with stakeholder-facing proof sections: an atomic-booking animation explaining advisory locks + `SKIP LOCKED`, feature proof cards for race-condition hardening, admin controls, notification-first workflow, and privacy-safe analytics. Added public `GET /api/event-pulse`, a CORS-enabled aggregate-only endpoint for live counts, booking status buckets, slot status buckets, company summaries, and integrity counters.
- **2026-06-06**: Moved the visualizer into `public/` so Vercel serves it at `/event-pulse.html`, and added a footer link labeled `Live` immediately after `Feedback`.
