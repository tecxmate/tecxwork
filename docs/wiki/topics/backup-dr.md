...
- **Linux:** user systemd — this repo includes `scripts/systemd/tecxwork-backup.service` and `scripts/systemd/tecxwork-backup.timer`, configured for Niko's Ubuntu checkout at `/home/niko/repos/tecxwork`, Node at `/home/niko/.nvm/versions/node/v20.20.2/bin/node`, and backups at `/home/niko/tecxwork-backups`. Install with:
  ...
  ```
  The timer runs 5 minutes after boot and then (by default) 1 hour after each completed run (`Persistent=true` catches missed runs). **On `niko-pc` the installed unit was changed to `OnUnitActiveSec=15min` on 2026-06-06** (every 15 minutes) — edit `~/.config/systemd/user/tecxwork-backup.timer`, `systemctl --user daemon-reload`, `restart tecxwork-backup.timer`. Note: the repo template `scripts/systemd/tecxwork-backup.timer` still ships hourly, so a reinstall from it reverts the interval. **Retention:** to keep the same ~48h window at 15-min cadence, `niko-pc`'s service sets `Environment=DB_RETENTION=192` (192 dumps × 15min ≈ 48h; ~64 MB total at ~0.33 MB/dump). Default elsewhere is 48 dumps. The service has systemd conditions for `/home/niko/repos/tecxwork/.env.local`, `/usr/bin/pg_dump`, and `pg_dump` version 17+; it skips cleanly until secrets and a compatible PostgreSQL client are installed. Logs append to `~/tecxwork-backups/backup.log` and `backup.err.log`.
