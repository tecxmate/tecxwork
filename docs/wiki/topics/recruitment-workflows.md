...
4. **Application summary and optional cancellation**: `/profile` shows a student's application ledger: companies applied to, position, status, and interview/proposed time. Student cancellation controls are hidden and API-blocked unless admin enables `student_cancellation_enabled`.
...
- 2026-06-01: Added student-side cancellation from the `/profile` application ledger. Active applications/interviews (`pending`, `waitlisted`, `reschedule_proposed`, `accepted`) now show a Cancel action with confirmation copy. `DELETE /api/bookings/[id]` now records cancellation audit rows, notifies recruiters when students cancel, sends a recruiter email, and keeps slot release / waitlist promotion behavior.
...
- 2026-06-01: Fixed impossible reschedule proposals when recruiter chooses a time with no available interviewer slot. Live SSB case: booking 58 was proposed at `2026-06-06T06:45:00Z` (14:45 Asia/Taipei), but recruiter 21 had booked slots at 14:30 and next available slots at 14:50; no slot existed at 14:45, so student Accept correctly failed with "No available interviewer slot." The propose-time route now checks `slots.status = available` for the exact proposed time before notifying the student, logs `proposal_blocked_no_slot`, and the recruiter UI only enables "Suggest anyway" for applicant-busy conflicts, not slot-unavailable conflicts.
- 2026-06-01: Fixed application state drift when recruiters edit job postings. Student/recruiter job pages and duplicate-application checks previously matched bookings to jobs by editable `bookings.position === job_openings.title`; if a recruiter changed a job title/content, students could see the job as not applied and apply again, and waitlisted/pending states were labeled generically as "Applied." Added nullable `bookings.job_opening_id`, a backfill script that links exact current title matches, stable id matching in student UI/API, duplicate blocking for pending/accepted/waitlisted/reschedule-proposed applications, and explicit Pending review / Waitlisted / Interview confirmed labels. Live backfill linked 124 of 145 bookings, including all 7 SSB bookings to job opening 47.
